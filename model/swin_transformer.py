...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, patch_size=4, in_chans=3, num_classes=1000,
        ...
        #     nn.Conv2d(int(embed_dim), 1, kernel_size=1, stride=1)
    ...
    def forward(self, x):
        ...
        for layer in self.layers:
            # print("each layer's x shape: ", x.shape)
            x, H, W = layer(x, H, W)
        ...
        # x = self.upsampling(x)
        # print("x upsampling shape: ", x.shape)
        # x = self.output(x)
        ...
        x = x.reshape(-1, channels, img_h, img_w)
        return x
...
#  csqi add from swin_transformer(simMIM源码)
